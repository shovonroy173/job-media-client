...
// console.log(location.state , pathType);
    ...
    <>
      ...
      <div className="px-14 w-full  ">
        ...
        <div className="flex flex-col justify-between pt-36 ">
          ...
          <p className="text-4xl font-extrabold">{location.state}</p>
          ...
          {currentItems.length > 0 ? currentItems.map((item, index) => (
            ...
            >
              <img
                src={item.urls[0]}
                alt=""
                className="w-[300px] object-cover rounded-xl " onClick={()=>navigate(`/gig/${pathType}/${item._id}`)}
              />
              <FavoriteRoundedIcon className="absolute top-3 right-3  rounded-full text-white hover:scale-150 hover:shadow-box-shadow2 hover:cursor-pointer" onClick={()=>{handleClick(item._id)}} />
              <div className="flex flex-col justify-around h-[150px]" onClick={()=>navigate(`/gig/${pathType}/${item._id}`)}>
